...
touching `lamp/index.html`.

An oil lamp — trim the wick's shape and height, and the flame follows. **The fuel is built** (Aug 2026): it burns down, you cannot see how much is left, and you find out by tapping the copper base with a ten penny nail lying beside it

**Done** was given for the wick and flame; the nail and the fuel came after, so the mark predates them and she has not run the piece since.
...
attached. At 78 the reflection has no seed and the same flood gives a clean nail.
...
self-consistent choice for a piece where the flame is what you can see and what you adjust.

**The tap is steel on copper**: a hard pitchless contact tick first, because that is
...
two minutes later, **791 at 1.62s** nearly dry.

**The tap only worked on the LEFT of the base** — her report off the machines, Aug 2026. The nail's
...
approaches (left and right, high and low): **two of four tapped before, four of four after**.

**And WHERE you tap earns its place**, which the old
...
really can be found by running the nail down the side, the way you find it on a gas cylinder.
...
Two things came out of being able
...
across), since it is no longer sized to leave room it never needed.

**The music button was lifted
