...
a gear's centre hole is where the axle goes and its size is determined, so a model with no notion
that the hole is *for* anything draws a plausible one; and overlap is the same blindness to two
gears being solid objects that cannot share a space.
...
| `birds/index.html` | ✅ |  | **Birds** perched on wires strung between two poles against a sunset-sky photo; the wires sag realistically and dip under whichever bird is sitting on them<br>**THE CEILING ON THIS PIECE IS HERS AND IT IS A TOOL, NOT A DESIGN.** Sep 2026: *"i think, realistically, i need to learn blender before birds can be really right. but i can still test ants and the behaviors of birds."* Read it exactly as `rain`'s qualifier is read — it is not *there is nothing more to want here*, it is *what is left wants a tool I haven't got*. **So don't propose rebuilding the birds to chase the remainder**, and don't offer to approximate an animated bird with more drawing: she has already weighed that and named what it would actually take. What IS in scope is everything the birds DO — how they land, sit, shuffle, take off, how the wire answers them, how they space themselves along it. Her reports on this piece will divide the same way, and the behaviour half is the half to act on.<br>Swept Aug 2026. Its two sliders were **18px** under a thumb and are now 30. And below 300px the
...
| `kaleidoscope/index.html` | ✅ | ✅ | A tray of real photographed small objects — glass, gems, gears, beads — mirrored live. Place them, then turn the ring<br>Objects re-cropped and the desktop controls spread Aug 2026. The turning ring (top) is **drawn, not an image** — brass-bound wood with a grab knob, dimmed so it doesn't fight the mirrored view. **The tray ring (bottom) is deliberately left brighter than the scope ring** — her call: the bright one pulls the eye first and says *drop things here*, then you look up and the dim ring's view makes sense. Don't 'fix' the mismatch; it is the wordless instruction. **AND THE DEEPER REASON FOR THAT ORDER, hers, Sep 2026, which was not written down when the decision was:** *"when you put your own objects in, you learned what the tiny watch gear looked like whole, and it's magical that when only part of it is reflected and re-reflected it looks like another thing entirely."* She is old enough to have had the fillable kind before the sealed plastic ones, and **the whole difference is that you knew the object first.** The magic is the DISTANCE between the two states, so a sealed kaleidoscope cannot have it — there is no before, only a pattern. The tray is therefore not a picker with a view above it; it is **half the experience**, and the brightness order is what puts the two states in sequence: see the thing, then see what it becomes. Take a fragment of anything and repeat it and it stops being itself — symmetry destroys recognition, which is the reason a kaleidoscope works at all and the reason only the person who supplied the gear gets the full of it. Anything that would let a visitor reach the scope without having seen the objects whole would cost the piece its point. **Done** — her verdict came on Mon 24 Aug,
...
all**, and the front was a plain ramp with no notion of what it was seen against.<br>**The first
...
looks as grey as one close and to the side."***<br>**So the dark side is GLARE and not geometry, and
...
0.22 -> 0.20** (all but unchanged, which is the point), and the sides unmoved at 0.5.<br>**Both
...
two branches meet at exactly mid grey and cannot do otherwise.<br>**The distance falloff is a square
...
far edge, where she wants grey.<br>**And the shading was in ELEVEN STEPS**, her other question the
...
slower did it cost about a tenth, which is a machine that does not exist.<br>**Comparing two builds
...
places in both, and a pixel diff over the whole frame then shows only the colour.<br>**WHAT IS BEHIND THE BULB IS SEEN THROUGH THE GLARE; WHAT IS IN FRONT IS NOT.** Hers, Sep 2026:
...
| `smoke.html` | — | — | **Smoke** — a started piece, not linked and not finished. Thirteen commits of history: it went ribbon → laminar-to-turbulent plume → true ribbon renderer → and finally a **2D fluid simulation** (Stable-Fluids with vorticity confinement and buoyancy) rendered as a self-shadowed volume in a light shaft. **Her ruling, Sep 2026: it stopped because it *"couldn't be made to look convincing. which is exactly why i need to learn blender."*** So this is `birds`' and `rain`'s judgement again and it is the clearest case of the three — the physics was taken a long way and the *look* is what defeated it. **Don't restart it, don't propose another renderer for it, and don't offer to finish it**: she has already worked out what it needs and it is not another pass at the drawing. It is a parked piece waiting on a tool, not an abandoned one. **NOTE it is on `main`, not on a side branch** (she believed otherwise), so like `crystal.html` and the chest drafts it is deployed and reachable at `/smoke.html` — nothing links to it, and that is the same footing the other unlinked drafts are on. |
